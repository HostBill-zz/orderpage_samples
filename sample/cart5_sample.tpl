            ...
            <tbody>
                ...
                {if $contents[2] }
                    {foreach from=$contents[2] item=domenka key=kk}{if $domenka.action!='own' && $domenka.action!='hostname'}
                            <tr >
                                <td>
                                    <strong>{if $domenka.action=='register'}{$lang.domainregister}{elseif $domenka.action=='transfer'}{$lang.domaintransfer}{elseif $domenka.action=='renew'}{$lang.domainrenewal}{/if}</strong> - {$domenka.name} - {$domenka.period} {$lang.years}
                                    <br/>
                                </td>
                                <td  align="center">-</td>
                ...
                <tr >
                    <td align="left" style="border:none">
                        {if $subtotal.coupon}
                        {else}
                            <div ><a href="#" onclick="$(this).hide();$('#promocode').show();return false;"><strong>{$lang.usecoupon}</strong></a></div>
                            <div id="promocode" style="display:none;">
                                <form action="" method="post" id="promoform" >
                                    <input type="hidden" name="addcoupon" value="true" />
                                    Code: <input type="text" class="styled" name="promocode"/> 
...
<form action="" method="post" onsubmit='if($("#c_notes").val()=="{$lang.c_tarea}")$("#c_notes").val("");$("#checkout").addLoader();'>
    ...
    <div class="wbox">
        ...
        <div class="wbox_content">
            <table border="0" cellpadding="0" cellspacing="6" width="100%">
                ...
                {if $tos}
                    <tr>
                        <td align="center"><input type="checkbox" value="1" name="tos"/> {$lang.tos1} <a href="{$tos}" target="_blank">{$lang.tos2}</a></td>
                    </tr>
                {/if}
            </table>
        </div>
    ...
    <div class="orderbox"  id="checkout">
        <div class="orderboxpadding">
            <center>
                <input type="submit" value="{$lang.checkout}" style="font-size:12px;font-weight:bold"  class="padded btn"/>
